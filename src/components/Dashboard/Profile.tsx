...
const Profile: React.FC = () => {
  ...
  useEffect(() => {
    const fetchUserData = async () => {
      ...
      if (token) {
        try {
          const response = await fetch('http://134.122.22.165/users/me', {
            method: 'GET',
            headers: {
  ...
  const handleFileUpload = async () => {
    if (selectedFile) {
      ...
      try {
        ...
        const response = await fetch('http://134.122.22.165/users/me/profile-picture', {
          method: 'POST',
          headers: {
            'Authorization': `Bearer ${token}`,
          },
  ...
  const addPoints = async (amount: number) => {
    ...
    if (token) {
      try {
        const response = await fetch('http://134.122.22.165/users/me/add-points', {
          method: 'POST',
          headers: {
            'Authorization': `Bearer ${token}`,
